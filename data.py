...
class DAVE2(Dataset):
    def __init__(self, root_dir, transform=None, type_='train'):
        self.root_dir = root_dir
        self.type = type_
        self.data = self.get_data()
        # self.images = self.create_image_list()
    ...
    def get_data(self):
        data_info = []
        if self.type == 'train':
            df = pd.read_csv('./data/DAVE2/train/train_data.csv')
            df['frame'] = df['frame'].apply(str)
            return df

        elif self.type == 'test':
            df = pd.read_csv('./data/DAVE2/test/test_data.csv')
            df['frame'] = df['frame'].apply(str)
            return df
    ...
    #     steering_csv_list = []
    #     # for hmb in self.hmb_list:
    #     df = pd.read_csv(os.path.join(self.root_dir, 'train_data.csv'))
    #     df['timestamp'] = df['timestamp'].apply(str)
    #     steering_csv_list.append(df)
    #
    #     steer_list = pd.concat(steering_csv_list)
    ...
    def __getitem__(self, idx):
        ...
        if self.type == 'train':
            ...
            steer = self.data.iloc[idx, 1]
            # # hmb = self.images.iloc[idx, 1]
            img_address = os.path.join(self.root_dir, 'train/'+image_name+'.jpg')
            image = io.imread(img_address)
            sample = {'image': image, 'steer': np.array([steer])}
            ...
            image_name = self.data.iloc[idx, 0] + '.jpg'
            steer = self.data.iloc[idx, 1]
            img_address = os.path.join(self.root_dir, 'test/'+image_name)
            image = io.imread(img_address)
            sample = {'image': image, 'steer': np.array([steer])}
...
if __name__ == "__main__":
    ...
    #dataset = DAVE2('./data/DAVE2', transform=composed)
    #index = DAVE2.__getitem__(dataset,idx=3)
    print(dataset)
    print(len(dataset))
